...
class PointingDiscourseGoldsegmentationModel(nn.Module):
    """
    ...
    def decode(self, words, feats, edu_break, beam_size=1):
        ...
        fencepost, span_split, decoder_init_state = self.encoder(words, feats)
        batch_size, seq_len = words.shape
        _, edu_len=edu_break.shape
        padded_zero = edu_break.new_zeros(batch_size)
        edu_break_tensor = torch.cat([padded_zero.unsqueeze(-1), edu_break], dim=1)
        fencepost_edu = fencepost[torch.arange(batch_size).unsqueeze(1), edu_break_tensor]
        span_split_edu = span_split[torch.arange(batch_size).unsqueeze(1), edu_break_tensor]
        enc_lens = edu_break_tensor.ne(0).sum(-1) + 1
        enc_len = int(enc_lens.max())
        dec_len = enc_len-1
        # lens = words.ne(self.args.pad_index).sum(1) - 1
        node_lens=enc_lens-2
        # print(lens)

        # pointing mask
        mask_l = enc_lens.new_tensor(range(0, enc_len)) > 0
        mask_r = enc_lens.new_tensor(range(0, enc_len)) < (enc_lens).view(-1, 1, 1)
        mask_point = ~(mask_l & mask_r)
        ...
        num_hyp = 1
        stacked_inputspan = enc_lens.new_zeros(batch_size, num_hyp, 2, dec_len)
        stacked_parsing_order = enc_lens.new_zeros(batch_size, num_hyp, 3, dec_len)
        stacked_inputspan[:, 0, 1, 0] = enc_lens - 1
        ...
        for t in range(num_steps):
            ...
            point_range_l = enc_lens.new_tensor(range(0, enc_len)) > \
                            curr_input_l.unsqueeze(-1).expand(batch_size, num_hyp, enc_len)
            point_range_r = enc_lens.new_tensor(range(0, enc_len)) < \
                            curr_input_r.unsqueeze(-1).expand(batch_size, num_hyp, enc_len)
            point_range = ~(point_range_l & point_range_r)
            mask_decodelens = (t >= node_lens).view(batch_size, 1, 1).expand(batch_size, num_hyp, enc_len)

            span_l = fencepost_edu[torch.arange(batch_size).unsqueeze(1), curr_input_l]
            span_r = fencepost_edu[torch.arange(batch_size).unsqueeze(1), curr_input_r]
            span_l = self.mlp_span_l_decoder(span_l)
            span_r = self.mlp_span_r_decoder(span_r)
            decoder_input = torch.cat([span_l, span_r], dim=-1)
            ...
            s_point = s_point.masked_fill(mask_point.expand(batch_size, num_hyp, enc_len), float('-inf'))
            s_point = F.log_softmax(s_point, dim=-1)
            # print(s_point[0])
            ...
            batch_index = enc_lens.new_tensor(range(batch_size)).view(batch_size, 1)
        ...
        final_stacked_parsing_order_edu = edu_break_tensor.unsqueeze(1).expand(batch_size, 3, edu_break_tensor.size(1))
        final_stacked_parsing_order_edu = final_stacked_parsing_order_edu.gather(dim=-1,
        ...
        # final_parsing_order = final_stacked_parsing_order[:,:,:max_parsing_length]

        l_lelf_point = fencepost[torch.arange(batch_size).unsqueeze(1), final_stacked_parsing_order_edu[:, 0, :]]
        l_split_point = fencepost[torch.arange(batch_size).unsqueeze(1), final_stacked_parsing_order_edu[:, 1, :]]
        l_right_point = fencepost[torch.arange(batch_size).unsqueeze(1), final_stacked_parsing_order_edu[:, 2, :]]
